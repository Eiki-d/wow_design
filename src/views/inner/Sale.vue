<template>
  <div>
      ...
      <ul v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="100" infinite-scroll-immediate-check="false">
          <router-link v-for="data in datalist" :key="data.productId" tag="li" @click="handleClick(data.productId)" to="/detail/:id" class="list">
...
export default {
    ...
    //   console.log('到底了！！！')
      ...
      Axios({
        url:  `/pages/category/${localStorage.getItem("asd")||this.$route.params.myid}?currentPage=${this.current}&sort=sales&order=desc&_=1577330411878`
      }).then(res => {
